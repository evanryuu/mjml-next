...
import { debounce, mjmlConvert } from './utils'
...
export default (editor: Editor, opt: RequiredPluginOptions) => {
  ...
  const coreMjmlModel = {
    init() {
      ...
      for (const prop in style) {
        if (!(prop in attrs)) attrs[prop] = style[prop]
      }
    ...
    getAttrToHTML() {
      ...
      for (const prop in attr) {
        ...
        if (value && value === style[prop]) delete attr[prop]
      }
  ...
  const coreMjmlView = {
    ...
    renderChildren(appendChildren: boolean) {
      ...
      for (let i = 0, len = childNodes.length; i < len; i++) container.appendChild(childNodes.shift())
    },

    checkVisibility() {
      if (this.model.isHidden?.()) this.el.style.display = 'none'
    },
